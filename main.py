...
offset = 0 # Para manejar los mensajes que recibe el bot
...
def enviar_telegram(mensaje, canal_id, con_boton=False, es_bienvenida=False):
    ...
    if es_bienvenida:
        payload["reply_markup"] = {
            "inline_keyboard": [
                [{"text": "🚀 Canal de Señal VIP", "url": LINK_CANAL_PRINCIPAL}],
                [{"text": "📈 Bitácora de Resultados", "url": LINK_BITACORA}]
            ]
        }
            ...
            if "message" in update and "text" in update["message"]:
                ...
                if texto == "/start":
                    bienvenida = (f"¡Bienvenido a *{BOT_NAME}*!\n\n"
                                  "Has ingresado al centro de mando de nuestro algoritmo de alta precisión. "
                                  "Analizamos el mercado 24/7 para enviarte las mejores oportunidades.\n\n"
                                  "👇 *Unete a nuestras comunidades:*")
                    enviar_telegram(bienvenida, chat_id, es_bienvenida=True)
    except: pass
...
while True:
    manejar_mensajes() # Revisa si alguien le dio a /start
    if esta_en_horario():
        if not sesion_anunciada:
            enviar_telegram(f"🔔 *ATENCIÓN TRADERS*\n\nLa sesión de {BOT_NAME} ha comenzado.", CANAL_PRINCIPAL)
